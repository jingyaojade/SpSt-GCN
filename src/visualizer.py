...
class Visualizer():
    ...
    def read_data(self):
        ...
        self.location = data['location'][self.args.visualization_sample]
        self.data = data['data'][self.args.visualization_sample]
        self.label = data['label']
        self.Ad = data['Ad'][self.args.visualization_sample]
        weight = data['weight']
        out = data['out']
        cm = data['cm']
    ...
    def show_NTU_skeleton(self):
        ...
        for t in frames:
            ...
            plt.title('sample:{}, frame:{}\n probablity:{:2.2f}%, pred_class:{}, actural_class:{}'.format(
                self.names[self.args.visualization_class-1], t, self.probablity*100, self.pred_class, self.actural_class
            ))
            ...
            for m in range(M):
                ...
                for v in range(V):
                    ...
                    # if v in [3,15,19,21,23]:
                    #     for edge in [3,15,19,21,23]:
                    #         if edge != v:
                    #             plt.plot([x[v], x[edge]], [y[v], y[edge]], '-o', color='red', linewidth=100*self.Ad[v][edge], markersize=0)
