...
class Processor(Initializer):
    ...
    def train(self, epoch):
        ...
        for num, (x, y, Ad, _) in enumerate(train_iter):
            self.optimizer.zero_grad()

            # Using GPU
            x = x.float().to(self.device)
            y = y.long().to(self.device)
            Ad = Ad.float().to(self.device)

            # Calculating Output
            out, _ = self.model(x,Ad)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            for num, (x, y, Ad, _) in enumerate(eval_iter):
                ...
                # Calculating Output
                out, _ = self.model(x, Ad)
                score_frag.append(out.data.cpu().numpy())  # for score fusion
            ...
            with open('D:/publication1/ss_GCN/{}_score.pkl'.format(self.args.config), 'wb') as f:
                pickle.dump(score, f)
    ...
    def extract(self):
        ...
        # Loading Data
        x, y, Ad, names = iter(self.eval_loader).__next__()
        location = self.location_loader.load(names) if self.location_loader else []

        # Calculating Output
        self.model.eval()
        out, feature = self.model(x.float().to(self.device), Ad.float().to(self.device))

        # Processing Data
        data, label, Ad = x.numpy(), y.numpy(), Ad.numpy()
        ...
        if not self.args.debug:
            ...
            np.savez('./visualization/extraction_{}.npz'.format(self.args.config),
                data=data, label=label, Ad=Ad, name=names, out=out, cm=self.cm,
