...
class Initializer():
    ...
    def init_dataloader(self):
        ...
        self.feeders, self.data_shape, self.Ad_shape, self.num_class, self.A, self.parts = dataset.create(
            self.args.dataset, **dataset_args
        )
        self.train_loader = DataLoader(self.feeders['train'],
            batch_size=self.train_batch_size, num_workers=0,
            pin_memory=True, shuffle=True, drop_last=True
        )
        self.eval_loader = DataLoader(self.feeders['eval'],
            batch_size=self.eval_batch_size, num_workers=0,
    ...
    def prepare_input(self):
        x1 = torch.FloatTensor([1, 6, 300, 25, 2])
        x2 = torch.FloatTensor([1, 25, 25])
        return dict(x=x1, x2=x2)

    def init_model(self):
        kwargs = {
            'data_shape': self.data_shape,
            'Ad_shape': self.Ad_shape,
        ...
        flops, params = thop.profile(deepcopy(self.model), inputs=(torch.rand([1,1]+self.data_shape[1:]), torch.rand([1]+self.Ad_shape)))
