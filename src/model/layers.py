...
class Spatial_Graph_Layer(nn.Module):  # 在这个module里要完成spatial和structural的合并
    def __init__(self, in_channel, out_channel, max_graph_distance, bias, act, residual=True, **kwargs):
        super(Spatial_Graph_Layer, self).__init__()

        self.conv_spatial = SpatialGraphConv(in_channel, out_channel, max_graph_distance, bias, **kwargs)
        self.conv_structural = StructuralGraphConv(in_channel, out_channel, max_graph_distance, bias, **kwargs)
        self.bn = nn.BatchNorm2d(out_channel)
        self.act = act
    ...
    def forward(self, x, Ad):
        res = self.residual(x)
        data = self.act(self.bn(self.conv_spatial(x)) + res)  # + self.conv_structural(x, Ad)
        return data
...
class SpatialGraphConv(nn.Module):
    def __init__(self, in_channel, out_channel, max_graph_distance, bias, edge, A, **kwargs):
        ...
        if edge:
            self.edge_spatial = nn.Parameter(torch.ones_like(self.A))
        else:
            self.edge_spatial = 1
        self.B_spatial = nn.Parameter(torch.zeros_like(self.A))

    def forward(self, x):
        x = self.gcn(x)
        n, kc, t, v = x.size()
        x = x.view(n, self.s_kernel_size, kc//self.s_kernel_size, t, v)
        x = torch.einsum('nkctv,kvw->nctw', (x, self.A * self.edge_spatial + self.B_spatial)).contiguous()
        return x


class StructuralGraphConv(nn.Module):
    def __init__(self, in_channel, out_channel, max_graph_distance, bias, edge, **kwargs):
        super(StructuralGraphConv, self).__init__()

        self.s_kernel_size = max_graph_distance + 1
        self.gcn = nn.Conv2d(in_channel, out_channel, 1, bias=bias)
        # self.gcn_main = nn.Conv2d(in_channel, out_channel * self.s_kernel_size, 1, bias=bias)
        if edge:
            self.edge_structural = nn.Parameter(torch.ones(25,25))
        else:
            self.edge_structural = 1

    def forward(self, x, Ad):
        x = self.gcn(x)   # input branch
        Ad = torch.Tensor(Ad)
        Ad_2 = torch.zeros_like(Ad)  # second person
        Ad = torch.cat((Ad, Ad_2), dim=0)
        x = torch.einsum('nctv,nvw->nctw', (x, Ad * self.edge_structural)).contiguous()
        return x  # 越相似，越要减